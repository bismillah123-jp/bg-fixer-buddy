-- Fix rollover function to include metadata (color) and other important fields
...
BEGIN
  ...
  WHERE stock_entries.metadata IS NULL OR stock_entries.metadata = '{}'::jsonb;
END;
$function$;

-- Also fix cascade_recalc_stock to preserve metadata
    ...
    WHILE v_current_date <= p_to_date LOOP
      ...
      BEGIN
        IF v_current_date > p_from_date THEN
          SELECT COALESCE(night_stock, 0), COALESCE(metadata, '{}'::jsonb), COALESCE(cost_price, 0)
          INTO v_prev_night_stock, v_metadata, v_cost_price
          FROM stock_entries
          ...
          LIMIT 1;
        ELSE
          SELECT COALESCE(morning_stock, 0), COALESCE(metadata, '{}'::jsonb), COALESCE(cost_price, 0)
          INTO v_prev_night_stock, v_metadata, v_cost_price
          FROM stock_entries
        ...
        -- Get metadata from stock_events if not found
        ...
        INSERT INTO stock_entries (
          ...
          COALESCE(v_metadata, '{}'::jsonb), COALESCE(v_cost_price, 0),
          NOW(), NOW()
        )
        ON CONFLICT (date, location_id, phone_model_id, COALESCE(imei, '')) 
        DO UPDATE SET
